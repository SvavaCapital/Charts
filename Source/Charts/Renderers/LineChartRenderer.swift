...
open class LineChartRenderer: LineRadarRenderer
{
    ...
    func drawGradientLine(context: CGContext, dataSet: ILineChartDataSet, spline: CGPath, matrix: CGAffineTransform)
    {
        ...
        let gradientStart = CGPoint(x: boundingBox.minX, y: 0)
        let gradientEnd = CGPoint(x: boundingBox.maxX, y: 0)
        var gradientColorComponents: [CGFloat] = []
        var gradientLocations: [CGFloat] = []
        
        for position in gradientPositions.reversed()
        {
            let location = CGPoint(x: position, y: boundingBox.minY)
                .applying(matrix)
            let normalizedLocation =
                (location.x - boundingBox.minX) / (boundingBox.maxX - boundingBox.minX)
